...
def comparemultiple(listofnames):
    ...
    html = df.style.set_table_styles(styles).apply(background_with_norm, subset = pd.IndexSlice[0:len(listofnames),["Three Pointers Made",  "Fieldgoal Percentage", "Freethrow Percentage", "Rebounds","Assists", "Steals", "Blocks","Points"]]).apply(background_with_normto, subset = pd.IndexSlice[0:len(listofnames),["Turnovers"]]).to_html()
